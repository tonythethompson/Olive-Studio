...
- Task 19 — Arena slot convenience sources (Req 18) — Olive outputs picker + Assistant OpenAI-compat snapshot; not implemented. Gate on Tasks **19.1–19.5** (shareable helpers, server Olive-output scan + opaque-ID map, snapshot eligibility/access control, ArenaPanel wiring, and Vitest coverage across server/unit/component + PBT Properties 20/20b/21/21b/22). Prerequisites: Task 3 (`mountArenaRoutes` + access boundary) and Task 6 (`ArenaPanel`).
