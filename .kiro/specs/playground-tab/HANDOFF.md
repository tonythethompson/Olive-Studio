...
- **Arena convenience sources (Req 18) are fill helpers, not new slot types.** Local stays file-based: server scans `cacheDir` + `output_dir` roots, client fetches bytes into a Session_Scoped `File`. Cloud stays OpenAI-compat fields: one-click snapshots the active Assistant provider only when it is OpenAI-compatible; otherwise soft-fail with a reason. Snapshot copy (editable after click), not live bind. Path sandbox rejects traversal outside those roots. See design.md "Requirement 18 Additions."

## Property tests to not lose track of

22 correctness properties total, spread across design.md (Properties 20–22 cover Req 18). Each has a `// Feature: playground-tab, Property N` tag convention and a minimum-100-iterations requirement (fast-check). Property numbers are non-contiguous by section (8, then 9 twice in different sections — pre-existing doc quirk, not a numbering error to "fix"; 13–19 were added earlier; 20–22 added with Req 18). Cross-reference tasks.md for which task number implements which property before assuming one is missing.
