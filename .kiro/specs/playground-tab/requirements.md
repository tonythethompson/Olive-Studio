...
4. THE server-side list and file endpoints SHALL sandbox reads to Olive_Output_Roots only, with separate list vs download contracts:
   ...
   - **Download (`GET /api/arena/olive-outputs/file`)**: THE server SHALL resolve the opaque id to a candidate path, then re-validate root containment, regular-file status, allowed extension (`.onnx` / `.ort`), and response-size limits before reading or streaming any bytes.
