...
4. THE server-side list and file endpoints SHALL sandbox reads to Olive_Output_Roots only. Before listing or streaming, THE server SHALL resolve the opaque id to a candidate path, re-validate containment inside a canonical root, confirm the target is a regular file, confirm an allowed extension (`.onnx` / `.ort`), and enforce a response-size limit. IF the id is unknown, the path escapes roots (including symlink escape), the file is not a regular model file, or the size exceeds the limit, THEN THE server SHALL return `403` (or `400`) with a clear error and SHALL NOT read or stream any bytes. THE server SHALL reject client-supplied `path`, `absolutePath`, `cacheDir`, and `outputDir` query parameters for these endpoints.
