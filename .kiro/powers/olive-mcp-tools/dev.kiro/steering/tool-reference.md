...
Run the setup script from the repo root (handles venv creation, deps, and verification):
...
The MCP server supports hybrid keyword + semantic search for documentation queries and troubleshooting. This requires `sentence-transformers` (installed automatically by the setup script).

**Pre-built indexes** ship in `olive_mcp_server/knowledge_base/indexes/` and work without any extra steps. To rebuild after modifying knowledge base files:
