...
| `SYNC_KB_TOKEN` / `VITE_SYNC_KB_TOKEN` | Optional shared secret for `POST /api/mcp/sync-kb`. When both are set to the same value, the client must send the matching `x-sync-token` header. |
