...
/** Scoring preference selector + Compare Results trigger, with disabled-state tooltip. */
function CompareControls({
  ...
  scoringSelectId: string;
  scoringPreference: ScoringPreference;
  onScoringChange: (preference: ScoringPreference) => void;
  canCompare: boolean;
  comparing: boolean;
    ...
    <div className="flex items-center gap-3 flex-wrap">
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          onChange={(e) => onScoringChange(e.target.value as ScoringPreference)}
      ...
      {compareError && <span className="text-xs text-rose-400 font-medium">{compareError}</span>}
    </div>
  );
}

/** MCP compare_results metric table, winner reasoning, and excluded-jobs report. */
function CompareResultsSection({
  compareResults,
  records,
}: {
  compareResults: CompareResultsOutput;
  records: JobHistoryRecord[];
}) {
  return (
...
/** Sortable table of the selected job history records with duration deltas. */
function RecordsTable({
  ...
  sorted: JobHistoryRecord[];
  onSort: (key: SortKey) => void;
  baseline: JobHistoryRecord | undefined;
  compareResults?: CompareResultsOutput | null;
}) {
  return (
    <div className="overflow-x-auto">
...
export function BatchComparisonView({
  ...
  const showCompareResults =
    compareResults != null && (compareResults.results.length > 0 || compareResults.winner === null);
  ...
  return (
    <div className="bg-slate-950/60 border border-electric-blue/30 rounded-xl p-5 space-y-4">
      ...
      {onCompare && (
        <CompareControls
          scoringSelectId={scoringSelectId}
          scoringPreference={scoringPreference}
          onScoringChange={setScoringPreference}
          canCompare={canCompare}
          comparing={comparing}
          effectiveCompletedCount={effectiveCompletedCount}
      ...
      {showCompareResults && (
        <CompareResultsSection compareResults={compareResults!} records={records} />
      )}
