...
export function InputInspector({ state }: InspectorProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <h3 className="text-sm font-semibold text-slate-300 mb-1.5 flex items-center gap-1">
        ...
        <button
          ...
          className="mt-2 text-[11px] text-electric-blue hover:text-white underline underline-offset-2 cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 gap-3 border-l border-slate-800/50 pl-4">
        <div className="rounded border border-slate-800 bg-slate-950/80 px-3 py-2">
          <p className="text-[11px] font-mono text-slate-500 uppercase mb-1">Selected source</p>
          <p className="text-xs font-mono text-slate-200">{sourceLabel}</p>
        </div>
        <div className="rounded border border-slate-800 bg-slate-950/80 px-3 py-2">
          <p className="text-[11px] font-mono text-slate-500 uppercase mb-1">Current model</p>
          <p className="text-xs font-mono text-slate-300 truncate" title={getModelSourceSummary(state)}>
