...
export function DevinAccountPanel({ providers }: DevinAccountPanelProps) {
  ...
  return (
    <div className="space-y-3 p-3 rounded-xl border border-slate-800 bg-slate-950/50">
      ...
      {devinMessage && <p className="text-xs text-emerald-400/90">{devinMessage}</p>}
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          disabled={devinBusy}
          onClick={() => void providers.handleDevinOpenSignIn()}
          className="flex-1 min-w-[8rem] h-9 bg-electric-blue hover:bg-electric-blue/90 disabled:opacity-40 rounded-lg text-sm font-bold text-white flex items-center justify-center gap-2"
        >
        ...
        <button
          ...
          className="h-9 px-3 border border-slate-700 rounded-lg text-sm text-slate-300"
        >
          Refresh
        </button>
        <button
          ...
          className="h-9 px-3 border border-rose-500/30 rounded-lg text-sm text-rose-400"
        >
          Logout
        </button>
      </div>
      <div>
        <label htmlFor="gemini-settings-devin-token" className="text-sm text-slate-400 mb-1 block">
        ...
        <button
          ...
          className="mt-2 w-full h-9 border border-electric-blue/40 text-electric-blue hover:bg-electric-blue/10 disabled:opacity-40 rounded-lg text-sm font-bold"
        >
          Complete sign-in
        </button>
      </div>
      {devinStatus?.signedIn && (
        <button
          ...
          className="w-full h-9 bg-electric-blue hover:bg-electric-blue/90 disabled:opacity-40 rounded-lg text-sm font-bold text-white flex items-center justify-center gap-2"
        >
          {isSavingProvider ? (
