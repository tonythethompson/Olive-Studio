    ...
    <div className="space-y-4">
      {analysis && !isAnalyzing && (
        <div className="bg-slate-950/70 rounded border border-slate-800 flex items-center gap-4 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 leading-relaxed mt-1">{analysis.summary}</p>
          </div>
        </div>
      )}
      ...
      {isAnalyzing && (
        <div className="text-center py-12 bg-slate-950/30 border border-slate-800 rounded-lg flex flex-col items-center justify-center">
          ...
          <p className="text-sm font-medium text-slate-300">Auditing pipeline...</p>
          <p className="text-sm text-slate-500 mt-0.5">Inspecting workspace…</p>
        </div>
      )}
      ...
      {analysis && !isAnalyzing && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-500 font-medium">Suggestions</span>
            <button
              type="button"
              onClick={onRunAnalysis}
              className="text-[11px] text-electric-blue hover:text-white flex items-center gap-1 cursor-pointer font-bold"
            >
              ...
              analysis.suggestions.map((s, i) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed break-words">{s.description}</p>
                  </div>
                  {s.autofix?.pass && (
                    <div className="pt-2 border-t border-slate-900/60 flex flex-wrap items-center gap-2 justify-between min-w-0">
                      ...
                      <button
                        ...
                        className="shrink-0 bg-electric-blue/10 text-electric-blue hover:bg-electric-blue hover:text-white border border-electric-blue/30 text-[11px] px-2.5 py-1 rounded font-bold inline-flex items-center gap-1 transition-all cursor-pointer"
                      >
      ...
      <button
        ...
        className="w-full h-10 bg-slate-950 hover:bg-slate-900 border border-slate-800 hover:border-slate-700 text-sm text-slate-200 font-bold flex items-center justify-center gap-2 rounded-lg cursor-pointer transition-colors"
      >
